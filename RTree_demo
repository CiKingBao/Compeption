...
def get_wifis_test():
    ...
    for mei in csv_reader_1:
        ...
        if k == 1:
            continue
        #each = sorted(mei[5].split(";"), key=lambda x: x.split("|")[1], reverse=True)
        ...
        if each1 in dic[mei[2]]:
            ...
            wifi.append(1)
        #wifi.append(dic[mei[2]].index(each1))
        sys.stdout.write('generated:{0}/total:{1}\r'.format(k, 48932))
        sys.stdout.flush()
...
if __name__ =="__main__":
    ...
    x_train = pd.DataFrame([list_k,wifi,wifi_secend])
